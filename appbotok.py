...
Ct.domanda=["Salve! seleziona utente"]
...
def textbox(text, box="other",value=''):
    ...
    if box == "self":
        ...
        # if len(Ct.risposta)>5:  # se lo tolgo si possono selezionare altre paramtri
        #     thumbnail = html.Div([
        #
        #
        #     ]),
        #
        #     textbox =  textbox = dbc.Card(html.Div([
        #
        #
        #     ]), style={'display': 'none'}, body=True, color=color, inverse=inverse
        #     )
        #
        #
        #
        ...
        if Ct.user in Ct.Prodotto:
            ...
            html.Div([
                Ct.domanda[-1],
                dcc.Dropdown(Ct.listaparametri, id='demo-dropdown',style =  {  'background-color': color,  'border-color': '#808080',  'color': '#ffab00'}),
                html.Div(id='dd-output-container'),
            ]),
            ...
            images_files = [f for f in os.listdir(os.path.join(os.getcwd(),"foto")) if f.endswith('.jpg')]
            text = Ct.visualizza[-1]


            nomra = text[text.find('NORMA') :text.find('AZIONI')].strip().capitalize()
            ...
            if Ct.risposta[3] in images_files[0]:
             for i in images_files:
                pil_image = Image.open(os.path.join(os.getcwd(),'foto',i))
...
#     children=[
#         dbc.Input(id="user-input", placeholder="Scrivi al chatbot...", type="text"),
#         dbc.Button("Invia", id="submit"),
#     ],
# )
...
# html.A( dbc.Button(
#                 html.Span([html.I(className="bi bi-arrow-clockwise me-2"), "Refresh Page"]),
#                 color="danger", style={"text-align": "center", "font-weight": "bold",
#                                         "font-size": "25px"},
#                 id="cleanbtn", disabled=False,
#                 n_clicks=0),href='/', style={"text-decoration": "none"})
...
def update_display(chat_history):
    # print("update_display",chat_history)
     ...
     # if value in Ct.operatore:
     #    df = Ct.df
     #    df1 = df[(df['Operatore (utente)'] == Ct.risposta[0])]
     #    Ct.listaparametri = list(set(df1['Tipi Schede'].tolist()))  # Ct.TipiSchede
     #    Ct.df1 = df1
     #
     #    Ct.listaparametri = list(set(df1['Tipo Macchinario'].tolist()))
     #    Ct.domanda.append('Scegli il tipo di macchinario')
  ...
  #   print('update_display', Ct.risposta, chat_history, history, len(history), risposta, domanda)
     ...
     #                  ):
     #     if i % 2 == 0:
     #         l.append(textbox(x, box="AI"))
     #     elif i % 2 != 0  and len(Ct.risposta)<=6:
     #         l.append(textbox(x, box="self", value=value))
     ...
     # al ppsto del menu metto una scritta
     c=0
     cd=0
     ...
     for i in range(len(history)-1):
         ...
         print(i,i % 2 != 0 )
         if (  i % 2 != 0 and len(Ct.risposta)>0) or ( i % 2 != 0 and len(Ct.risposta) and  Ct.risposta[-1] in Ct.Parametro):
             ...
             # else:
             text =  Ct.risposta[c] #+ f'Hai selezionato il valore:  {Ct.risposta[c]}'  # qua in una variabile nel control magari metto la domanda,
             ...
             if Ct.domanda[ cd]=='Scegli il tipo di Parametro da visualizzare':
                 ...
                 nomra=text[text.find('ProtezioneCliente')+len('ProtezioneCliente'):text.find('AZIONI')].strip().capitalize()
                 azioni = text[text.find('AZIONI'):].strip().capitalize()
              ...
              text = Ct.domanda[ cd]  # + f'Hai selezionato il valore:  {Ct.risposta[c]}'  # qua in una variabile nel control magari metto la domanda,
...
def run_chatbot(value,chat_history):
    ...
    # if user is None:
    #
    #     return f"{Ct.domanda[-1]}<split>",''



    #chat_history += f"{  Ct.domanda[-1]}<split> { value }<split>"

    print("run_chatbot",chat_history,len(Ct.risposta))
    if len(Ct.risposta)<=5:
        chat_history += f"{  Ct.domanda[-1]}<split> { value }<split>"
